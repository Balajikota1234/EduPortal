  ...
  app.post(api.student.tests.submit.path, async (req, res) => {
    ...
    try {
      ...
      const result = await storage.createResult({
        ...
        isRead: false,
        answers: JSON.stringify(input.answers) // Store answers for review
      });
...
async function seedDatabase() {
  ...
  if (!anyAdmin) {
    // Only seed if there are zero admin accounts in the system.
    // Once you've created your own admin and deleted this default one,
    // this block will never run again.
    ...
    console.log("Created default admin user: admin / admin123 (no admin accounts existed)");
  }
}